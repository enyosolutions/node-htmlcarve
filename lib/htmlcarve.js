(function() {
  var Htmlcarve, buildLinkDerivats, chardet, cheerio, combineInfoPartials, extractKeywords, iconv, link, link2, link3, link4, link5, mergeObjects, parsePageBody, parser, request, splitToWords, URL,
  ...
  Htmlcarve.fromUrl = function(url, fn) {
    ...
    return request({
      ...
      return parsePageBody(error, response, body, function(error, page) {
        var data;
        data = Htmlcarve.fromString(page, url);
  ...
  Htmlcarve.fromString = function(text, url) {
    var $, data;
    $ = cheerio.load(text, {
    ...
    console.log(data);
      ...
      if (url){
          var site = URL.parse(url);
          if(site.host.match(/(www\.)?airbnb\..*$/)){
              data.source.airbnb = parser.airbnb.execute($);
          }
  ...
  parsePageBody = function(error, response, body, fn) {
    var coding, page;
    if (!response || !body) {
 -      console.log("error (http: " + response.statusCode + ") while parsing... ", error);		 +      console.log("error (empty response or body) while parsing... ");
 +    }
    else if (error || response.statusCode !== 200) {
      console.log("error (http: " +  (response ? response.statusCode :'')  + ") while parsing... ", error);
    }
    else{
    coding = chardet.detect(body.slice(0, 2500));
    try {
  ...
  combineInfoPartials = function(info) {
      ...
      var elements = {};
      for(var i in info){
          mergeObjects(elements, info[i]);
      }
    return (function(base) {
      mergeObjects(base, JSON.parse(JSON.stringify(info.html_meta)));
      mergeObjects(base, info.twitter_card);
      mergeObjects(base, info.open_graph);
      console.log(base);
      return base;
    })(elements);
  };
